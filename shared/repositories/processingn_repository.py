...
from shared.db.models.index_models import ProcessingnModelDb

class ProcessingnRepository:
    @staticmethod
    def get_documents(db: Session):
        try:
            results = db.query(ProcessingnModelDb).all()
    ...
    def insert_many_documents(db: Session, documents):
        try:
            db.bulk_insert_mappings(ProcessingnModelDb, documents)
            db.commit()
            return documents
        except Exception as e:
    ...
    def delete_documents(db: Session):
        try:
            db.query(ProcessingnModelDb).delete()
            db.commit()
        except Exception as e:
